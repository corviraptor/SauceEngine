...
public class PlayerCrouch : MonoBehaviour
{
    ...
    void Crouch(object sender, PlayerSettings player, int crouchState, bool isOnGround, RaycastHit hit){
        ...
        if (crouchState == 0 && isOnGround && PlayerMovementEvents.current.clocks["jumpBuffer"] == 0 && hit.point.y <= PlayerMovementEvents.current.center.y - (controller.height/2)){
        ...
        if (crouchState == 2 && isOnGround && PlayerMovementEvents.current.clocks["jumpBuffer"] == 0 && hit.point.y <= PlayerMovementEvents.current.center.y - (controller.height/2)){
